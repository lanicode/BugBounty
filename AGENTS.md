...
Der produktive Code in `apps/` und `packages/` bildet ausschließlich den Phase-1-Sicherheitskern. `legacy/mvp/` ist eine unsichere, nicht produktive Audit-Referenz.
...
- Keine aktiven Sicherheitstests, Kontoerstellung, Plattformintegration, Report-Einreichung oder LLM-gesteuerten Requests ergänzen.
