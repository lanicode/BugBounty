# Phase-4-Register externer Aktionen
...
oder nicht verifizierbare Store-Evidence blockieren. Es gibt weiterhin keinen
realen Adapter und keinen External-Action-Transport.

Jede simulationsfähige Action erzwingt diese vollständige Kette:
...
Die signierte `external_action`-Entscheidung bindet exakt den immutable
Approval-Binding-Digest. Credential, Signatur, Nonce, Session, Revision,
Control-Plane-ID und Kontext werden beim Entscheiden und erneut vor
Reservation, Runner-Start und erfolgreichem Settlement geprüft.

Die Registry-Secret-Art ist ausschließlich ein zukünftiges Typmetadatum. In
Phase 4 wird kein externes Secret angefordert, aufgelöst, gespeichert oder an
den Mock-Runner übergeben. Der lokale Ed25519-Operator-Key ist eine Control-
Plane-Signierfähigkeit und ausdrücklich kein Secret einer externen Action.
...
vorab registrierten In-Process-Antwort. Die Loopback-Adresse ist unveränderlich
eingefrorenes Registry-Metadatum, kein vom Runner ausgeführter Request.
...
Action `offline_inspect` beschränkt.
...
`external` wird unabhängig von Konfiguration, Signatur oder vorhandener
Evidence abgelehnt. Report- und Triage-Approvals bleiben lokale Datensätze ohne
externe Wirkung.

Während Entwicklung und Tests von Phase 4 wurde kein realer Plattform- oder
